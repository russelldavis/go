...
redo:
	...
	if rkIsLetter(c) || c >= utf8.RuneSelf && s.isIdentRune(c, true) {
		s.ident()
		return
	}
...
func (s *rkScanner) ident() {
	...
	// possibly a keyword
	if len(lit) >= 2 {
		if tok := rkKeywordMap[rkHash(lit)]; tok != 0 && rkTokStrFast(tok) == string(lit) {
