...
func (p *rkParser) init(file *PosBase, r io.Reader, errh ErrorHandler, pragh PragmaHandler, mode Mode) {
	p.file = file
	p.errh = errh
	p.mode = mode
	p.scanner.init(
...
func (p *rkParser) updateBase(pos Pos, tline, tcol uint, text string) {
	i, n, ok := trailingDigitsRk(text)
	if i == 0 {
	...
	var line, col uint
	i2, n2, ok2 := trailingDigitsRk(text[:i-1])
	if ok2 {
...
func commentTextRk(s string) string {
	if s[:2] == "/*" {
...
func trailingDigitsRk(text string) (uint, uint, bool) {
...
func (p *rkParser) got(tok token) bool {
	if p.tok == tok {
		p.next()
		return true
	}
...
func (p *rkParser) want(tok token) {
	if !p.got(tok) {
		p.syntaxError("expecting " + tokstringRk(tok))
		p.advance()
	}
}
...
func (p *rkParser) gotAssign() bool {
	switch p.tok {
	case _Define:
...
func (p *rkParser) posAt(line, col uint) Pos {
	return MakePos(p.base, line, col)
}

// error reports an error at the given position.
func (p *rkParser) errorAt(pos Pos, msg string) {
	err := Error{pos, msg}
	if p.first == nil {
		p.first = err
	}
...
func (p *rkParser) syntaxErrorAt(pos Pos, msg string) {
	if trace {
	...
	case _IncOp:
		...
		tok += tok
	default:
		tok = tokstringRk(p.tok)
	}

	p.errorAt(pos, "syntax error: unexpected "+tok+msg)
}

// tokstringRk returns the English word for selected punctuation tokens
// for more readable error messages.
func tokstringRk(tok token) string {
	switch tok {
	case _Comma:
		return "comma"
	case _Semi:
...
func (p *rkParser) pos() Pos               { return p.posAt(p.line, p.col) }
func (p *rkParser) syntaxError(msg string) { p.syntaxErrorAt(p.pos(), msg) }

// The stopset contains keywords that start a statement.
// They are good synchronization points in case of syntax
// errors and (usually) shouldn't be skipped over.
const stopsetRk uint64 = 1<<_Break |
	1<<_Const |
	1<<_Continue |
	1<<_Defer |
	1<<_Fallthrough |
...
func (p *rkParser) advance(followlist ...token) {
	if trace {
...
func (p *rkParser) trace(msg string) func() {
	p.print(msg + " (")
	const tab = ". "
...
func (p *rkParser) print(msg string) {
	fmt.Printf("%5d: %s%s\n", p.line, p.indent, msg)
}
...
func (p *rkParser) fileOrNil() *File {
	if trace {
		defer p.trace("file")()
	}
	...
	for p.tok != _EOF {
		...
		default:
			if p.tok == _Lbrace && len(f.DeclList) > 0 && isEmptyFuncDeclRk(f.DeclList[len(f.DeclList)-1]) {
...
func isEmptyFuncDeclRk(dcl Decl) bool {
	f, ok := dcl.(*FuncDecl)
...
//
func (p *rkParser) list(open, sep, close token, f func() bool) Pos {
	p.want(open)
	...
	for p.tok != _EOF && p.tok != close && !done {
		...
		if !p.got(sep) && p.tok != close {
			p.syntaxError(fmt.Sprintf("expecting %s or %s", tokstringRk(sep), tokstringRk(close)))
...
func (p *rkParser) appendGroup(list []Decl, f func(*Group) Decl) []Decl {
	if p.tok == _Lparen {
		g := new(Group)
...
func (p *rkParser) importDecl(group *Group) Decl {
	if trace {
...
func (p *rkParser) constDecl(group *Group) Decl {
	if trace {
...
func (p *rkParser) typeDecl(group *Group) Decl {
	if trace {
...
func (p *rkParser) varDecl(group *Group) Decl {
	if trace {
...
// Receiver     = Parameters .
func (p *rkParser) funcDeclOrNil() *FuncDecl {
	if trace {
...
func (p *rkParser) funcBody() *BlockStmt {
	p.fnest++
	errcnt := p.errcnt
	body := p.blockStmt("")
	p.fnest--
...
func (p *rkParser) binaryExpr(prec int) Expr {
...
func (p *rkParser) unaryExpr() Expr {
	if trace {
...
func (p *rkParser) callStmt() *CallStmt {
	if trace {
...
func (p *rkParser) operand(keep_parens bool) Expr {
	if trace {
...
func (p *rkParser) pexpr(keep_parens bool) Expr {
	if trace {
		defer p.trace("pexpr")()
	}
		...
		case _Dot:
			...
			case _Lparen:
				...
				if p.got(_Type) {
					...
					// t.Lhs is filled in by rkParser.simpleStmt
					t.pos = pos
					t.X = x
					x = t
				} else {
...
func (p *rkParser) bare_complitexpr() Expr {
	if trace {
...
func (p *rkParser) complitexpr() *CompositeLit {
	if trace {
...
func newIndirectRk(pos Pos, typ Expr) Expr {
	o := new(Operation)
	o.pos = pos
	o.Op = Mul
	o.X = typ
...
func (p *rkParser) typeOrNil() Expr {
	if trace {
	...
	case _Star:
		// ptrtype
		p.next()
		return newIndirectRk(pos, p.type_())
...
func (p *rkParser) funcType() *FuncType {
	if trace {
...
func (p *rkParser) dotname(name *Name) Expr {
	if trace {
...
func (p *rkParser) structType() *StructType {
	if trace {
...
func (p *rkParser) interfaceType() *InterfaceType {
	if trace {
...
func (p *rkParser) funcResult() []*Field {
	if trace {
...
func (p *rkParser) addField(styp *StructType, pos Pos, name *Name, typ Expr, tag *BasicLit) {
	if tag != nil {
...
// Tag            = string_lit .
func (p *rkParser) fieldDecl(styp *StructType) {
	if trace {
	...
	case _Lparen:
		...
		if p.tok == _Star {
			...
			pos := p.pos()
			p.next()
			typ := newIndirectRk(pos, p.qualifiedName(nil))
			p.want(_Rparen)
			tag := p.oliteral()
			p.addField(styp, pos, nil, typ, tag)
...
func (p *rkParser) oliteral() *BasicLit {
	if p.tok == _Literal {
		b := new(BasicLit)
		b.pos = p.pos()
		b.Value = p.lit
...
func (p *rkParser) methodDecl() *Field {
	if trace {
...
func (p *rkParser) paramDeclOrNil() *Field {
	if trace {
...
// ...Type
func (p *rkParser) dotsType() *DotsType {
	if trace {
...
func (p *rkParser) paramList() (list []*Field) {
	if trace {
...
// We represent x++, x-- as assignments x += ImplicitOneRk, x -= ImplicitOneRk.
// ImplicitOneRk should not be used elsewhere.
var ImplicitOneRk = &BasicLit{Value: "1"}

// SimpleStmt = EmptyStmt | ExpressionStmt | SendStmt | IncDecStmt | Assignment | ShortVarDecl .
func (p *rkParser) simpleStmt(lhs Expr, keyword token) SimpleStmt {
	if trace {
	...
	if _, ok := lhs.(*ListExpr); !ok && p.tok != _Assign && p.tok != _Define {
		...
		case _IncOp:
			...
			op := p.op
			p.next()
			return p.newAssignStmt(pos, op, lhs, ImplicitOneRk)
...
func (p *rkParser) newRangeClause(lhs Expr, def bool) *RangeClause {
	r := new(RangeClause)
	r.pos = p.pos()
...
func (p *rkParser) newAssignStmt(pos Pos, op Operator, lhs, rhs Expr) *AssignStmt {
	a := new(AssignStmt)
	a.pos = pos
	a.Op = op
	a.Lhs = lhs
...
func (p *rkParser) labeledStmtOrNil(label *Name) Stmt {
	if trace {
...
func (p *rkParser) blockStmt(context string) *BlockStmt {
	if trace {
...
func (p *rkParser) declStmt(f func(*Group) Decl) *DeclStmt {
	if trace {
...
func (p *rkParser) header(keyword token) (init SimpleStmt, cond Expr, post SimpleStmt) {
	p.want(keyword)
...
func (p *rkParser) switchStmt() *SwitchStmt {
	if trace {
...
func (p *rkParser) selectStmt() *SelectStmt {
	if trace {
...
func (p *rkParser) caseClause() *CaseClause {
	if trace {
...
func (p *rkParser) commClause() *CommClause {
	if trace {
...
// 	DeferStmt .
func (p *rkParser) stmtOrNil() Stmt {
	if trace {
...
func (p *rkParser) stmtList() (l []Stmt) {
	if trace {
...
// Arguments = "(" [ ( ExpressionList | Type [ "," ExpressionList ] ) [ "..." ] [ "," ] ] ")" .
func (p *rkParser) argList() (list []Expr, hasDots bool) {
	if trace {
...
func (p *rkParser) newName(value string) *Name {
	n := new(Name)
	n.pos = p.pos()
	n.Value = value
	return n
...
func (p *rkParser) nameList(first *Name) []*Name {
	if trace {
...
func (p *rkParser) qualifiedName(name *Name) Expr {
	if trace {
...
// unparenRk removes all parentheses around an expression.
func unparenRk(x Expr) Expr {
	for {
		p, ok := x.(*ParenExpr)
		if !ok {
			break
